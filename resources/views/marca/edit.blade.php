...
<div class="container-fluid px-4">
    ...
    <div class="container w-10 border border-3 border-primary rounded p-4 mt-3">
        ...
        <form action="{{route('marcas.update',['marca'=>$marca])}}" method="post" autocomplete="off">
            @method('PATCH')
            @csrf
            <div class="row g-3">
                ...
                <div class="col-md-12">
                    ...
                    <textarea name="descripcion" class="form-control" id="descripcion" rows="3">{{old('descripcion',$marca->caracteristica->descripcion)}}</textarea>
                    @error('descripcion')
                        <small class="text-danger"><strong>{{'*'.$message}}</strong></small>
                    @enderror
                </div>
